...
# untokenized XML/XLIFF tags and url addresses - high level technical specification 
# can be found at: http://code.google.com/p/m4loc/wiki/TableOfContents?tm=6 , 
# click on "Specifications" and select "ModTokenizer - Technical Specification". 
# Moses' tokenizer.perl and nonbreaking_prefixes direcory are required by the 
# script.
#
#
#
...
#set up all data streams for UTF-8 data
binmode( $tmpout, ":utf8" );
binmode(STDIN, ":utf8");
binmode(STDOUT, ":utf8");
...
#FUNCTIONS---------------------------------------------------------------------------------------------
  ...
  sub processNode {
	...
	#if this text node is in some placeholder - assign it to str_tag (will be tokenized)
	if($#in_ph==-1){
	    $str_tok .= $reader->value;
	}
	#otherwise assign it to str_tok (won't be tokenized)
	else{
	    $str_tag .=$reader->value;
	}
    }
    #else (if it is not textual string)
  ...
  sub tokenize{
    ...
    for(my $i=0;$i<=$#arr;$i++){
	switch(($i+1)%3){
	    case 0 {} # do nothing; or create a better regexp and remove this useless case ;)
	    case 1 {
...
L<https://docs.google.com/leaf?id=1bfmJwW5M1tODnzSTAt3Fg9HBM8Rr4ylg58SIKkUgPr0&sort=name&layout=list&pid=0BwHykr6vFJhWMjYzYTc2Y2YtOTFmYS00YTQzLTg1ZTQtNmE4OWFlODZmOTI5&cindex=7>
